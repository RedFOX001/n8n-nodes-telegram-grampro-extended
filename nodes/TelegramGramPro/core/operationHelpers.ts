...
export abstract class BaseOperation {
	...
	protected async initializeClient(): Promise<void> {
		const creds = (await this.context.getCredentials('telegramGramProApi')) as TelegramCredentials;
	 ...
	 */
	async run(): Promise<INodeExecutionData[]> {
...
export class ParameterExtractor {
	...
	static extractMediaParams(context: IExecuteFunctions, index: number = 0) {
		return {
			...
			media: context.getNodeParameter('media', index) as unknown,
