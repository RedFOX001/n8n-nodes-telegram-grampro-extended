...
function extractFloodWaitSeconds(message) {
    var _a;
    const match = (_a = message.match(/FLOOD_WAIT[_\s]*(\d+)/i)) !== null && _a !== void 0 ? _a : message.match(/\b(\d+)\b/);
    if (!match)
        return undefined;
...
function mapTelegramError(error) {
    var _a;
    const rawMessage = error instanceof Error ? error.message : String(error !== null && error !== void 0 ? error : 'Unknown error');
    ...
    if (normalized.includes('FLOOD_WAIT')) {
        const retryAfterSeconds = (_a = extractFloodWaitSeconds(rawMessage)) !== null && _a !== void 0 ? _a : 60;
        return {
            rawMessage,
            code: 'FLOOD_WAIT',
...
//# sourceMappingURL=telegramErrorMapper.js.map